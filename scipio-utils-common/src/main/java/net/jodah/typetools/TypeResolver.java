...
public final class TypeResolver {
  ...
  static {
    SUPPORTS_LAMBDAS = Double.parseDouble(System.getProperty("java.version").substring(0, 3)) >= 1.8;
    if (SUPPORTS_LAMBDAS) {
   ...
   * @return argument for {@code type} else {@link Unknown} if no type arguments are declared
   ...
   * @param subType to extract type variable information from
   * @return argument for {@code genericType} else {@link Unknown} if no type arguments are declared
