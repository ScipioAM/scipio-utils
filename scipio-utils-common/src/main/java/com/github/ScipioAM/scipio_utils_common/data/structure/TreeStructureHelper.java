...
public class TreeStructureHelper {
     ...
     * @return 带有完整树型结构的根节点
     */
    public static <T extends TreeNode> T buildTreeForRoot(T root, List<T> nodeList) {
        //创建缓存
     ...
     */
    public static <T extends TreeNode> List<T> readRootTree(T root, TreeReadListener listener) {
     ...
     * @return 扁平的所有节点list <strong>（深度优先）</strong>
     */
    public static <T extends TreeNode> List<T> readForest(List<T> forest, TreeReadListener listener) {
     ...
     * @return 结果集
     */
    private static <T extends TreeNode> List<T> readTreeByDeep(List<T> forest, List<T> resultList, TreeReadListener listener) {
        if(forest!=null) {
            for(T node : forest) {
                resultList.add(node);
                //遍历读取时的回调
